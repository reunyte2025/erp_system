...
import { createQuotation, updateQuotationFull, getComplianceByCategory, getComplianceDescriptions } from '../../services/quotation';
...
// ============================================================================
// COMPLIANCE CONSTANTS — EXECUTION ONLY (Water Main + STP)
// ============================================================================
...
// ============================================================================
// SUCCESS MODAL — same as quotations.jsx
// ============================================================================

const SuccessModal = ({ isOpen, onClose, onViewOrder, orderNumber, orderId, isEditMode }) => {
  useEffect(() => {
    if (isOpen) {
  ...
  const fmtNum = (n) => {
    if (!n) return '';
    if (String(n).startsWith('PO-')) return String(n);
    const s = String(n);
    if (s.length >= 8) return `PO-${s.substring(0, 4)}-${s.substring(4).padStart(5, '0')}`;
    return `PO-2026-${String(n).padStart(5, '0')}`;
  };
...
export default function CreatePurchaseOrder({ onUpdateNavigation }) {
  ...
  // Breadcrumbs — plain strings like rest of the app
  useEffect(() => {
  ...
  // Draft - with proper error handling
  ...
  const saveDraft = (data) => {
    try {
      ...
      logger.log('[Draft] Purchase order draft saved successfully');
      return true;
    } catch (error) {
      console.warn('[PurchaseOrder Draft] Failed to save draft:', error.message);
      // Could add toast notification: toast.error('Draft could not be saved')
      return false;
    }
  };

  /**
   * Clear draft from localStorage with error handling
   * Returns true if successful, false if failed
   */
  const clearDraft = () => {
    try {
      localStorage.removeItem(DRAFT_KEY);
      logger.log('[Draft] Purchase order draft cleared successfully');
      return true;
    } catch (error) {
      console.warn('[PurchaseOrder Draft] Failed to clear draft:', error.message);
      return false;
    }
  };
  ...
  const loadDraft = () => {
    try {
      ...
      if (data) {
        logger.log('[Draft] Purchase order draft loaded successfully');
      }
      return data ? JSON.parse(data) : null;
    } catch (error) {
      console.warn('[PurchaseOrder Draft] Failed to load draft:', error.message);
      return null;
    }
  };
  ...
  const [_catLoading,             setCategoryComplianceLoading] = useState(false);
  ...
  useEffect(() => {
    ...
    if (draft) {
      ...
      // Always regenerate PO number — never reuse a stale/old number from draft
      setQuotationNumber(String(generatePONumber()));
    } else {
      setQuotationNumber(String(generatePONumber()));
    }
  ...
  // Auto-save draft
  useEffect(() => {
    saveDraft({ sections, sacCode, quotationNumber, gstEnabled, gstRate, discountType, discountValue });
  }, [sections, sacCode, quotationNumber, gstEnabled, gstRate, discountType, discountValue]); // eslint-disable-line react-hooks/exhaustive-deps

  const generatePONumber = () => {
    // Backend builds the full number e.g. "QT-202603-3093529"
    // We just pass the trailing unique integer part e.g. 3093529
    return Math.floor(1000000 + Math.random() * 9000000); // 7-digit unique number
  };
  ...
  const fetchComplianceByCategory = async (categoryId) => {
    ...
    try {
      ...
      if (response?.status === 'success' && response?.data) {
        ...
        setCategoryCompliance(arr.map(c => ({ id: c.id, name: c.name || c.compliance_name || '', compliance_name: c.name || c.compliance_name || '' })));
      } else { setCategoryCompliance([]); }
    } catch { setCategoryCompliance([]); }
    finally { setCategoryComplianceLoading(false); }
  };

  // Uses service function — no direct api call in JSX file
  ...
  const getUsedDescriptionsForSub = () => {
    const used = new Set();
  ...
  const handleSelectCategoryType = (type) => {
    ...
    setCategoryItemsMap(prev => { const n = { ...prev }; categoryIds.forEach(id => { if (!n[id]) n[id] = { items: [], category_name: COMPLIANCE_CATEGORIES[id].shortName }; }); return n; });
    setActiveCategoryId(firstCategoryId);
    setItemFormMap(prev => { const n = { ...prev }; categoryIds.forEach(id => { n[id] = { ...BLANK_ITEM_FORM }; }); return n; });
    setEditingItemIndexMap(prev => { const n = { ...prev }; categoryIds.forEach(id => { n[id] = null; }); return n; });
  ...
  const handleAddItem = () => {
    ...
    const newItem = {
      compliance_name: itemForm.compliance_name.trim(), compliance_id: itemForm.compliance_id || null,
      sub_compliance_id: null, quantity: parseInt(itemForm.quantity, 10) || 1,
      miscellaneous_amount: String(itemForm.miscellaneous_amount ?? '').trim(),
      Professional_amount: parseFloat(itemForm.Professional_amount) || 0, total_amount: calcItemTotal(itemForm),
    };
  ...
  const handleEditItem = (index) => {
    ...
    setActiveItemForm({ compliance_name: item.compliance_name, compliance_id: item.compliance_id || null, sub_compliance_id: null, quantity: item.quantity || 1, miscellaneous_amount: item.miscellaneous_amount || '', Professional_amount: item.Professional_amount || 0 });
  ...
  const handleAddSection = () => {
    ...
    if (editingSection !== null) {
      ...
      const mergedSection       = { ...sectionForm, items: updatedItemsFromMap !== undefined ? updatedItemsFromMap : sectionForm.items };
      ...
      setSections(prev => {
        ...
        Object.entries(categoryItemsMap).forEach(([catIdStr, catData]) => {
          ...
          if (existingIndex !== -1) {
            ...
            next = [...next, { category_id: catId, category_name: COMPLIANCE_CATEGORIES[catId]?.shortName || catData.category_name || `Category ${catId}`, items: newItems }];
          }
        });
        return next;
      });
  ...
  const handleEditSection = (index) => {
    ...
    setCategoryItemsMap({ [categoryId]: { items: [...section.items], category_name: section.category_name } });
  ...
  const handleEditItemFromTable = (sectionIndex, itemIndex) => {
    ...
    setItemFormMap({ [categoryId]: { compliance_name: item.compliance_name, compliance_id: item.compliance_id || null, sub_compliance_id: null, quantity: item.quantity, miscellaneous_amount: item.miscellaneous_amount || '', Professional_amount: item.Professional_amount || 0 } });
  ...
  const closeModal = () => {
    ...
    setEditingSection(null); setCategoryCompliance([]);
    setComplianceDescriptions([]); setDescriptionMode('dropdown'); setDescriptionSearch(''); setShowDescriptionDropdown(false);
    ...
    try {
      const allItems = sections.flatMap(section =>
        section.items.map((item) => {
          ...
          if (isNaN(Professional_amount) || Professional_amount <= 0) throw new Error(`Professional amount must be > 0 for: ${description}`);
          return {
      ...
      if (isEditMode) {
        const updatePayload = {
          ...
          client: null, vendor: vendorId, project: projectId,
          gst_rate:         String(gstEnabled ? (parseFloat(gstRate) || 0).toFixed(2) : '0'),
          discount_rate:    discountRateStr, sac_code: sacCode.trim(),
          total_amount: parseFloat(subTotal.toFixed(2)), total_gst_amount: parseFloat(gstAmt.toFixed(2)), grand_total: parseFloat(grandTotal.toFixed(2)),
          items: allItems,
        };
      ...
      const quotationNum = Number(quotationNumber) || generatePONumber();
      if (isNaN(quotationNum) || quotationNum <= 0) throw new Error('Invalid quotation number');

      const quotationData = {
        quotation_number: parseInt(quotationNum),
        client: null,       // VENDOR quotation — client always null
        vendor: vendorId,   // VENDOR quotation — vendor id
      ...
      console.log('📤 CREATE PO PAYLOAD:', JSON.stringify(quotationData, null, 2));
      const response = await createQuotation(quotationData);
      if (response.status === 'success' || response.data?.status === 'success') {
        setCreatedOrder(response.data || response);
        clearDraft(); setShowSuccessModal(true);
  ...
  // Filtered lists
  const filteredVendors  = vendors.filter(v => { const n = (v.name || v.company_name || '').toLowerCase(); const e = (v.email || '').toLowerCase(); return n.includes(vendorSearch.toLowerCase()) || e.includes(vendorSearch.toLowerCase()); });
  const filteredProjects = allProjects.filter(p => (p.name || p.title || '').toLowerCase().includes(projectSearch.toLowerCase()));

  // Modal computed vars — same pattern as quotations.jsx
  ...
  const modalUsedDescriptions = (() => {
    ...
    if (!isEditSectionMode) {
      getUsedDescriptionsForSub().forEach(d => used.add(d));
  ...
  const modalAddItem = isEditSectionMode
    ? () => {
        ...
        if (!itemForm.compliance_name.trim()) return;
        const newItem = { compliance_name: itemForm.compliance_name.trim(), compliance_id: itemForm.compliance_id || null, sub_compliance_id: null, quantity: parseInt(itemForm.quantity, 10) || 1, miscellaneous_amount: String(itemForm.miscellaneous_amount ?? '').trim(), Professional_amount: parseFloat(itemForm.Professional_amount) || 0, total_amount: calcItemTotal(itemForm) };
        ...
        modalSetItemForm({ ...BLANK_ITEM_FORM }); setDescriptionSearch(''); setShowDescriptionDropdown(false);
      }
    : handleAddItem;
  const modalEditItem = isEditSectionMode
    ? (index) => {
        ...
        if (!item) return;
        modalSetItemForm({ compliance_name: item.compliance_name, compliance_id: item.compliance_id || null, sub_compliance_id: null, quantity: item.quantity || 1, miscellaneous_amount: item.miscellaneous_amount || '', Professional_amount: item.Professional_amount || 0 });
        modalSetEditingIdx(index); fetchComplianceDescriptions(activeCategoryId, null);
      }
    : handleEditItem;
  ...
  const modalTotalItems   = isEditSectionMode ? (categoryItemsMap[activeCategoryId]?.items?.length || 0) : Object.values(categoryItemsMap).reduce((sum, cat) => sum + (cat.items?.length || 0), 0);
  const modalSaveDisabled = isEditSectionMode ? (categoryItemsMap[activeCategoryId]?.items?.length || 0) === 0 : Object.values(categoryItemsMap).every(cat => (cat.items?.length || 0) === 0);

  // ============================================================================
  // RENDER — exact same layout as quotations.jsx
  // ============================================================================
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-[1400px] mx-auto">
        ...
        {/* Header Card — same as quotations.jsx */}
          ...
          <div className="p-6">
            <div className="flex items-center justify-between">
                          ...
                          <div className="max-h-64 overflow-y-auto">
                            {projectsLoading ? (
                              ...
                              <button key={project.id} onClick={() => { setSelectedProject(project); setShowProjectDropdown(false); setProjectSearch(''); }}
                                ...
                                <div className="flex items-center gap-3">
                                  <div className="w-8 h-8 bg-purple-100 rounded-full flex items-center justify-center flex-shrink-0"><Building2 className="w-4 h-4 text-purple-600" /></div>
                                  <div className="flex-1 min-w-0">
                                    <div className="font-medium text-gray-900 text-sm truncate">{project.name || project.title}</div>
                                    <div className="text-xs text-gray-500 truncate">{[project.city, project.state].filter(Boolean).join(', ') || 'No location'}</div>
                                  </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <button onClick={handleSubmit} disabled={submitting || sections.length === 0 || !selectedVendor || !selectedProject}
        ...
        {isEditMode && (
          <div className="mb-3 bg-amber-50 border border-amber-200 rounded-xl px-4 py-3 flex items-center gap-3">
            <div className="w-8 h-8 bg-amber-100 rounded-lg flex items-center justify-center flex-shrink-0"><Edit className="w-4 h-4 text-amber-600" /></div>
        ...
        {/* Main Content — same 12-column grid as quotations.jsx */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
          ...
          <div className="lg:col-span-8">
            <div className="bg-white rounded-xl shadow-sm border border-gray-200">
              <div className="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 bg-teal-500 rounded-lg flex items-center justify-center"><FileText className="w-5 h-5 text-white" /></div>
                  <h2 className="text-lg font-semibold text-gray-900">Execution Compliance Builder</h2>
                </div>
                <button onClick={() => handleSelectCategoryType('execution')}
              ...
              {/* Table header — same as quotations.jsx */}
              <div className="px-6 py-3 bg-gray-50 border-b border-gray-200">
                <div className="grid grid-cols-12 gap-4">
                  <div className="col-span-3 text-xs font-semibold text-gray-600 uppercase">Compliance Type</div>
                  <div className="col-span-1 text-xs font-semibold text-gray-600 uppercase text-center">Qty</div>
                  <div className="col-span-3 text-xs font-semibold text-gray-600 uppercase text-center">Misc. Amount</div>
                  <div className="col-span-3 text-xs font-semibold text-gray-600 uppercase text-center">Professional Charges</div>
                        ...
                        <div className="bg-white">
                          {section.items.map((item, itemIndex) => (
                            <div key={itemIndex} className="grid grid-cols-12 gap-4 items-center px-4 py-3 border-b border-gray-100 last:border-0 hover:bg-gray-50 transition-colors group">
                              ...
                              <div className="col-span-3 text-center"><span className="text-sm text-gray-700">Rs. {parseFloat(item.Professional_amount || 0).toLocaleString('en-IN')}</span></div>
                              <div className="col-span-2 text-right flex items-center justify-end gap-1.5">
                                <span className="text-sm font-semibold text-gray-900">Rs. {calcItemTotal(item).toLocaleString('en-IN')}</span>
                                <button onClick={() => handleEditItemFromTable(sectionIndex, itemIndex)} className="opacity-0 group-hover:opacity-100 text-blue-500 hover:text-blue-600 p-1 hover:bg-blue-50 rounded transition-all" title="Edit item"><Edit className="w-3.5 h-3.5" /></button>
          ...
          {/* Right Column — identical to quotations.jsx */}
              ...
              <div className="p-6">
                ...
                <div className="mt-6 pt-6 border-t border-gray-200">
                  <div className="flex items-center gap-2 text-teal-600 mb-4">
                    <FileText className="w-4 h-4" /><span className="text-sm font-semibold">Quick Info</span>
                  </div>
                  <div className="space-y-2 text-sm">
                    ...
                    <div className="flex items-center justify-between"><span className="text-gray-600">Total Quantity</span><span className="font-medium text-gray-900">{sections.reduce((sum, s) => sum + s.items.reduce((is, i) => is + i.quantity, 0), 0)}</span></div>
                  </div>
                </div>
              </div>
            </div>
        ...
        {/* Add/Edit Compliance Modal — exact copy of quotations.jsx modal */}
        {showAddSection && (
                ...
                <div id="compliance-modal-body" className="p-5 space-y-4 bg-gray-50 flex-1" style={{ minHeight: 0, overflowY: 'scroll', scrollbarGutter: 'stable' }}>
                  ...
                  <div className="bg-white rounded-xl border border-gray-200 overflow-visible">
                    ...
                    <div className="p-4 space-y-3">
                      ...
                      <div className="relative" style={{ paddingBottom: showDescriptionDropdown ? '320px' : '0', transition: 'padding-bottom 0.15s ease' }}>
                        ...
                        {complianceDescLoading ? (
                          ...
                          <div className="description-dropdown relative w-full">
                            <button type="button"
                              onClick={() => { const next = !showDescriptionDropdown; setShowDescriptionDropdown(next); if (next) { setTimeout(() => { const body = document.getElementById('compliance-modal-body'); if (body) body.scrollTo({ top: body.scrollHeight, behavior: 'smooth' }); }, 50); } }}
                              ...
                              <span className={modalItemForm.compliance_name ? 'text-gray-900' : 'text-gray-500'}>
                                {modalItemForm.compliance_name ? (modalItemForm.compliance_name.length > 65 ? modalItemForm.compliance_name.substring(0, 65) + '...' : modalItemForm.compliance_name) : 'Select a description'}
                              </span>
                              ...
                              <div className="absolute left-0 top-full mt-2 w-full bg-white rounded-lg shadow-xl border border-gray-200 overflow-hidden" style={{ zIndex: 99999 }} onMouseDown={(e) => e.preventDefault()}>
                                <div className="p-3 border-b border-gray-100 bg-gradient-to-b from-gray-50 to-white">
                                  ...
                                  <div className="flex items-center justify-between mt-2 px-0.5">
                                    <span className="text-xs text-gray-400">{complianceDescriptions.filter(d => !modalUsedDescriptions.has(d.compliance_description) && d.compliance_description?.toLowerCase().includes(descriptionSearch.toLowerCase())).length} results</span>
                                    ...
                                    .map((desc, idx) => {
                                      ...
                                      return (
                                        <button key={desc.id} type="button"
                                          onClick={() => { modalSetItemForm(prev => ({ ...prev, compliance_name: desc.compliance_description })); setShowDescriptionDropdown(false); setDescriptionSearch(''); }}
                          ...
                          <textarea value={modalItemForm.compliance_name} onChange={e => modalSetItemForm(prev => ({ ...prev, compliance_name: e.target.value }))}
                      ...
                      {/* Qty / Professional / Misc — same as quotations.jsx */}
                      <div className="grid grid-cols-3 gap-3">
                        <div>
                          ...
                          <input type="number" min="1" value={modalItemForm.quantity} onChange={e => modalSetItemForm(prev => ({ ...prev, quantity: parseInt(e.target.value) || 1 }))}
                        ...
                        <div>
                          ...
                          <input type="number" min="0" step="0.01" value={modalItemForm.Professional_amount === 0 ? '' : modalItemForm.Professional_amount}
                            onChange={e => modalSetItemForm(prev => ({ ...prev, Professional_amount: parseFloat(e.target.value) || 0 }))} placeholder="0.00"
                        ...
                        <div>
                          ...
                          <input type="text" value={modalItemForm.miscellaneous_amount} onChange={e => modalSetItemForm(prev => ({ ...prev, miscellaneous_amount: e.target.value }))}
                      ...
                      {/* Item Total + Add button — identical to quotations.jsx */}
                      <div className="flex items-center gap-3">
                        ...
                        <button onClick={modalAddItem} disabled={!modalItemForm.compliance_name.trim() || !(parseFloat(modalItemForm.Professional_amount) > 0)}
                  ...
                  {/* Items list — identical to quotations.jsx */}
                  ...
                  {/* Cross-category hint */}
                  {!editingSection && (categoryItemsMap[activeCategoryId === 3 ? 4 : 3]?.items?.length > 0) && (
      ...
      {/* Bottom Action Bar — same as quotations.jsx */}
        ...
        <div className="p-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              {sections.length > 0 && (
                <button onClick={() => { if (window.confirm('Clear all draft data?')) { clearDraft(); setSections([]); setSelectedVendor(null); setSelectedProject(null); setSacCode(''); setGstRate(18); setDiscountValue(0); setQuotationNumber(String(generatePONumber())); } }}
      ...
      <SuccessModal
        ...
        onClose={() => {
          ...
          if (!isEditMode) { setSections([]); setSelectedVendor(null); setSelectedProject(null); setSacCode(''); setGstRate(18); setDiscountValue(0); clearDraft(); setQuotationNumber(String(generatePONumber())); }
        }}
        onViewOrder={() => {
